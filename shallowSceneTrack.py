...
attribs=dfKnownOverlap[['Places365 Resnet50 Image Classification','Places365 Resnet50 Percentage']].to_numpy()

errorList=[]
rInd=0
for row in dfScenes:
  try:
    xMat[rInd,20:]=dfPCA[dfPCA.index==row].to_numpy()[:,:30] #:10]
  except:
    errorList.append(row)
  rInd+=1

print('errorList: ')
print(str(len(errorList))+' of '+str(xMat.shape[0]))
...
if permScenes:
  ...
  for sceneInd in range(len(picNumsPerm)):
    ...
    attribsCopy[list(range(k,k+len(currPicRows))),:]=attribs[currPicRows,:]
...
for fold in range(9):
  ...
  truePredDict[fold]=np.vstack((yLabels,newLabels))
  attribDict[fold]=attribs[testInd,:]
